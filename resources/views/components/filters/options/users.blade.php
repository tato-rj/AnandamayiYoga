<div class="d-flex">
	<form class="mr-2">
	    <select class="form-control" name="order" onchange="this.form.submit()">
	        <option selected disabled>@lang('Order by')</option>
	        <option value="name" @filter('order', 'name') selected @endfilter>@lang('Name')</option>
	        <option value="newest" @filter('order', 'newest') selected @endfilter>@lang('Newest')</option>
	        <option value="oldest" @filter('order', 'oldest') selected @endfilter>@lang('Oldest')</option>
	    </select>
	</form>
	<form>
	    <select class="form-control" name="status" onchange="this.form.submit()">
	        <option selected disabled>@lang('Show only')</option>
