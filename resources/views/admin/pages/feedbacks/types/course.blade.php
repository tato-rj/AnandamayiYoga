<div class="col-12">
  <p>We have <strong>{{\App\Feedback::for('program')->count()}}</strong> {{str_plural('rating', \App\Feedback::for('program')->count())}} on the courses.</p>

  @if(! \App\Feedback::for('course')->get()->isEmpty())
	<div>
	  ...
	  @foreach(\App\Feedback::for('course')->get() as $feedback)
	    ...
	    <div class="d-flex justify-content-between border-bottom cursor-pointer hover-background-light">
	      <div class="d-flex">
	        ...
	        <div class="p-2" style="width: 168px">
	          <span class="clamp-1"><small>{{\App\Course::where('id', $feedback->target_id)->first()->name}}</small></span>
	        </div>
